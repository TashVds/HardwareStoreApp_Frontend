...
export default {
  ...
  methods: {
    async register() {
      try {
        const { data } = await axios.post('http://localhost:8087/api/register', this.customer);
        this.result = data;
    ...
    togglePasswordVisibility() {
      this.customer.passwordFieldType = this.customer.passwordFieldType === 'password' ? 'text' : 'password';
    },
    cancel() {
      window.history.back();
    }
